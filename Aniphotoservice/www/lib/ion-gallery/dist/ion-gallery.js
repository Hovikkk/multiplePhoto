...
(function(){
  ...
  ionSlider.$inject = ['$ionicModal', 'ionGalleryHelper', '$ionicPlatform', '$timeout', '$ionicScrollDelegate', '$ionicSlideBoxDelegate', 'dataStorage', '$state', '$rootScope'];

  function ionSlider($ionicModal, ionGalleryHelper, $ionicPlatform, $timeout, $ionicScrollDelegate, $ionicSlideBoxDelegate, dataStorage, $state, $rootScope){
    ...
    function controller($scope){
      ...
      $scope.deleteImageFunction = function (ind) {
          $scope.afterDelete = []
          console.log(ind)
          var data = this;
          ...
          for (var i = 0; i < $scope.items.length; i++) {
              for (var j = 0; j < $scope.items[i].length; j++) {
                  if (ind != $scope.items[i][j].id) {
                      count++;
                      if (it[I].length == 3) {
                          I++;
                          it.push([]);
                      }
                      it[I].push($scope.items[i][j])
                  } 
              }
          }
          $scope.items = []//it;
          */
          var count = 0;
          for (var i = 0; i < $scope.ionGalleryItems.length; i++) {
              if ($scope.ionGalleryItems[i].id == ind){
                  k = i
                  count++
                  break;
              }
          ...
          if (k>-1){
              $scope.ionGalleryItems.splice(k, 1);
          }
          ...
          //var removed = $scope.items[removeI].splice(removeJ, 1);
          //console.log($scope.items)
          //localStorage.removeItem("picture");
          //$scope.items = $scope.afterDelete
         //localStorage.setItem("picture", JSON.stringify(it))
         ...
         /* $scope.it = []
          for (var k = 0; k < it.length; k++) {
              for (var n = 0; n < it[k].length; n++) {
                  $scope.it.push(it[k])
              }
          }
          console.log($scope.it)
          localStorage.setItem("picture", $scope.it)*/
         // console.log($scope.items)
       //   $state.reload()
      }
      ...
      $scope.showImage = function (index) {
         /* var d = 0;
          ...
          for (var k = 0; k < $scope.items.length; k++) {
              for (var p = 0; p < $scope.items[k].length; p++) {
                  $scope.newItems.push($scope.items[k][p])
              }
          }
          console.log($scope.newItems)
          $scope.ionGalleryItems = $scope.newItems
